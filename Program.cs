...
//                         }
                        ...
                        using (StreamReader sr = new StreamReader("Tiago.txt"))
                        {
                            string linha;
...
// CNPJ: {cadaItem.cnpj}, Válido: {(metodosPj.validarCnpj(cadaItem.cnpj) ? "sim" : "não")}
...
// Imposto devido: {metodosPj.PagarImposto(cadaItem.rendimento).ToString("C")}
// ");
